...
volatile float pressure, temperature, humidity;
uint16_t size;
int Data[512];
int buffer[90];

char buffertosave[128];
...
void TIM3_IRQHandler(void)
{
	...
	sprintf((char *)Data,"AT+S.HTTPGET=192.168.1.30,/node3/save?temperature=%.2f&pressure=%.2f&humidity=%.1f,3000\r",
...
int main(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8, GPIO_PIN_RESET); //M1 = 0
  HAL_Delay(10);
	  ...
	  if(IsDataAvailable(lora_uart))
	  	{
	  		for(int i=0;i<94;++i)
	  		{
		  		...
		  		if(Look_for("three-run",buffertosave) == 1) // If "three-run" command is received via LoRa, turn on the LED
		  		{
		  			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
		  			memset(buffertosave,0,128);
		  			break;
		  		}
		  		else if(Look_for("three-stop",buffertosave) == 1) // If "three-stop" command is received via LoRa, turn off the LED
		  		{
